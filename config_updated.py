"""Enhanced configuration with database and alert settings."""
import os
...
class Config:
    ...
    WEBHOOK_URL: Optional[str] = os.getenv('WEBHOOK_URL')  # Slack, Discord, etc.
    ...
    # Alerts
    ALERT_ON_TRADE: bool = os.getenv('ALERT_ON_TRADE', 'true').lower() == 'true'
    ALERT_ON_STOP_LOSS: bool = os.getenv('ALERT_ON_STOP_LOSS', 'true').lower() == 'true'
    ALERT_ON_TAKE_PROFIT: bool = os.getenv('ALERT_ON_TAKE_PROFIT', 'true').lower() == 'true'
    ALERT_ON_ERROR: bool = os.getenv('ALERT_ON_ERROR', 'true').lower() == 'true'
    DAILY_SUMMARY_ENABLED: bool = os.getenv('DAILY_SUMMARY_ENABLED', 'true').lower() == 'true'
    DAILY_SUMMARY_TIME: str = os.getenv('DAILY_SUMMARY_TIME', '18:00')  # 6 PM
    ...
    def log_config(cls) -> None:
        ...
        logger.info(f"Dashboard: {cls.DASHBOARD_ENABLED}")
        logger.info(f"Database: {cls.DATABASE_URL}")
        logger.info(f"Email Alerts: {'Enabled' if cls.EMAIL_FROM else 'Disabled'}")
        logger.info(f"Webhook Alerts: {'Enabled' if cls.WEBHOOK_URL else 'Disabled'}")
        logger.info("=" * 50)
